...
// ★ Mobile/external lot tracking (FIXED V2)
...
bool   g_mobile_grid_building = false; // ★ Flag: grid currently being built (prevent re-entry)
datetime g_mobile_last_check_time = 0; // ★ Last check timestamp to prevent rapid re-checks
int    g_mobile_consecutive_fails = 0; // ★ Count consecutive failures to prevent infinite retry
...
// ★ FIXED: Magic-filtered close (only EA positions, NOT mobile)
void CloseAllPositionsForSymbol(const string sym)
{
   for(int i=PositionsTotal()-1; i>=0; --i)
   {
      ...
      // ★ CRITICAL: Only close OUR positions (skip mobile/external)
      long mag = PositionGetInteger(POSITION_MAGIC);
      if(mag != MagicGrid && mag != MagicMarket) continue;
...
// ★ FIXED: Magic-filtered delete with retry logic
void DeleteAllPendingOrdersForSymbol(const string sym)
{
   for(int i=OrdersTotal()-1; i>=0; --i)
   {
      ...
      // ★ CRITICAL: Only delete OUR orders (skip external)
      long mag = OrderGetInteger(ORDER_MAGIC);
      if(mag != MagicGrid && mag != MagicMarket) continue;

      // ★ Retry logic for failed deletes
      int retry = 3;
      bool ok = false;
      while(retry > 0 && !ok)
      {
         ...
         if(!ok)
         {
            PrintFormat("OrderDelete failed [%I64u] attempt %d: %s", ticket, 4-retry, trade.ResultRetcodeDescription());
            Sleep(100);
            retry--;
         }
      }
...
void OpenPositionsEx(ENUM_ORDER_TYPE mode, bool useAutoLotFlag, bool useBottomGridFlag)
{
   ...
   if(mode == ORDER_TYPE_BUY)
   {
      ...
      if(useBottomGridFlag && !use_mobile_base)  // ★ Skip validation if using mobile position
      {
         if(bottom_p <= 0.0)
         {
         ...
         // ★ Validate bottom price is reasonable (ONLY when NOT using mobile)
   ...
   else if(mode == ORDER_TYPE_SELL)
   {
      ...
      if(useBottomGridFlag && !use_mobile_base)  // ★ Skip validation if using mobile position
      {
         if(top_far <= 0.0)
         {
            ...
            if(idx >= 0) 
            {
               top_far = iHigh(_Symbol, PERIOD_CURRENT, idx);
            }
            else
            {
               PrintFormat("⚠️ Bottom Grid SELL: iHighest failed, cannot determine top price");
               return; // ★ Abort if top price cannot be determined
            }
         }
         
         // ★ Validate top price is reasonable (ONLY when NOT using mobile)
         if(top_far <= 0.0 || top_far <= bid)
         {
            PrintFormat("⚠️ Bottom Grid SELL: Invalid top price %.5f (bid=%.5f) - aborting", top_far, bid);
            return;
         }
      }
      else if(use_mobile_base)
      {
         // ★ For mobile position: use mobile entry as top price
         top_far = mobile_entry_price;
         PrintFormat("✅ Using mobile entry %.5f as top price (skipping validation)", top_far);
      }
...
void MarketBuyOnce()
{
   // ★ FIXED V2: Check building flag, cooldown AND our positions BEFORE routing to grid
   if(g_mobile_base_lot > 0.02 && !g_mobile_grid_building && g_mobile_check_cooldown <= 0 && !HasOurPositionsOrOrders())
   {
      PrintFormat("MarketBuy: Mobile lot %.2f detected (cooldown=0, not building, no our orders) -> routing through Bottom Grid Buy", g_mobile_base_lot);
      
      // ★ Set flags before grid building
      g_mobile_grid_building = true;
      g_mobile_grid_built = true;
      g_mobile_check_cooldown = 50;
...
void MarketSellOnce()
{
   ...
   if(g_mobile_base_lot > 0.02 && !g_mobile_grid_building && g_mobile_check_cooldown <= 0 && !HasOurPositionsOrOrders())
   {
      PrintFormat("MarketSell: Mobile lot %.2f detected (cooldown=0, not building, no our orders) -> routing through Bottom Grid Sell", g_mobile_base_lot);
...
int OnInit()
{
   ...
   g_mobile_check_cooldown = 10;  // ★ Start with cooldown to allow EA initialization
   g_mobile_grid_building = false;
   g_mobile_last_check_time = 0;
...
//─────────────────────────────────────────────────────────────
// Trade transaction: COMPREHENSIVE liquidation detection
//─────────────────────────────────────────────────────────────
                        ...
                        const MqlTradeResult&     result)
{
      ...
      trans.type == TRADE_TRANSACTION_REQUEST)
   {
      if(StringLen(trans.symbol) && trans.symbol == _Symbol)
      {
         ...
         // ★ Count EA positions and total volume
         int our_position_count = 0;
         double our_total_volume = 0.0;
         ...
         for(int i=PositionsTotal()-1; i>=0; --i)
         {
            ...
            {
               our_position_count++;
               our_total_volume += PositionGetDouble(POSITION_VOLUME);
            }
         }
         
         // ★ Check if mobile position still exists
         ...
         if(g_mobile_base_lot > 0.02)
         {
            ...
            double lot = 0.0;
            ulong ticket = 0;
            mobile_exists = FindExternalPosition(is_buy, price, lot, ticket);
         }
         
         // ★ TRIGGER 1: All EA positions closed
         bool trigger_no_positions = (our_position_count == 0);
         
         // ★ TRIGGER 2: Mobile position closed (grid should be cleared)
         bool trigger_mobile_closed = (g_mobile_base_lot > 0.02 && !mobile_exists);
         
         // ★ TRIGGER 3: Significant volume reduction (>80% closed)
         bool trigger_volume_drop = false;
         if(g_grid_last_index > 0 && g_grid_step_used > 0.0)
         {
            // Calculate expected volume from last grid build
            int n = g_grid_last_index + 1;
            double base_lot = GetFirstGridLotRaw();
            for(int i=0; i<n; i++)
            {
               expected_volume += NormalizeLot(base_lot * MathPow(AutoLotRatio, i));
            }
            
            if(expected_volume > 0.0)
            {
               double remaining_pct = (our_total_volume / expected_volume) * 100.0;
               trigger_volume_drop = (remaining_pct < 20.0); // <20% remaining
            }
         }
         
         // ★ Execute cleanup if ANY trigger activated
         if(trigger_no_positions || trigger_mobile_closed || trigger_volume_drop)
         {
            string reason = "";
            if(trigger_no_positions) reason += "NO_EA_POSITIONS ";
            if(trigger_mobile_closed) reason += "MOBILE_CLOSED ";
            if(trigger_volume_drop) reason += "VOLUME_DROP ";
            
            PrintFormat("★ Liquidation detected [%s] - cleaning up limit orders", reason);
            
            // ★ Delete ALL EA pending orders with retry
            int deleted_count = 0;
            for(int i=OrdersTotal()-1; i>=0; --i)
            {
               ulong ticket = OrderGetTicket(i);
            ...
            if(deleted_count > 0)
            {
               PrintFormat("★ Cleanup complete: deleted %d pending limit orders", deleted_count);
               ...
               {
                  // Mobile gone - full reset
                  g_mobile_base_lot = 0.0;
                  g_mobile_ticket = 0;
                  g_mobile_grid_built = false;
                  g_mobile_grid_building = false;
                  g_mobile_check_cooldown = 0;
                  PrintFormat("Grid and mobile position cleared - full reset");
               }
            }
         }
      }
...
// ★ FIXED V3: Comprehensive mobile grid stability with time-based throttling
void CheckAndUpdateMobileLot()
{
   // ★ TIME-BASED THROTTLING: prevent checks within 1 second (reduced from 3)
   datetime now = TimeCurrent();
   if(now - g_mobile_last_check_time < 1)  // ★ Reduced to 1 second
   {
      return; // Skip if checked within last 1 second
   }
   ...
   // ★ CRITICAL: Skip if grid is currently being built
   if(g_mobile_grid_building)
   {
      return;
   }
   
   // ★ Update last check time
   g_mobile_last_check_time = now;
   
   // ★ Wait for server synchronization after recent trades
   Sleep(200); // ← Ensure server state is synchronized
   ...
   if(FindExternalPosition(is_buy, price, lot, ticket))
   {
      ...
      if(lot > 0.02)
      {
         ...
         if(ticket != g_mobile_ticket || g_mobile_base_lot == 0.0)
         {
            ...
            g_mobile_consecutive_fails = 0; // ★ Reset failure counter for new position
            ...
            if(HasOurPositionsOrOrders())
            {
               ...
               Sleep(500); // Wait for close/delete operations to complete
               
               g_has_target_sl = false;
               ResetTPState();
               ObjectDelete(0, OBJ_TP1_LINE);
               
               // ★ Set cooldown to prevent immediate grid build
               g_mobile_check_cooldown = 20; // Wait 20 ticks (~2 seconds) before building grid
               
               PrintFormat("Old grid cleaned up - waiting before building new grid");
               return; // ← Exit early to allow cooldown
            }
         ...
         // ★ FIXED V3: Comprehensive auto-build with retry logic and verification
         // Conditions:
         // 1. Grid not built yet
         // 2. No our positions/orders exist
         // 3. No active cooldown
         // 4. NOT currently building
         // 5. Consecutive failures < 3 (prevent infinite retry)
         if(!g_mobile_grid_built && !HasOurPositionsOrOrders() && g_mobile_check_cooldown <= 0 && !g_mobile_grid_building && g_mobile_consecutive_fails < 3)
         {
            PrintFormat("Auto-building grid for mobile position: %.2f lot %s (attempt %d)", lot, (is_buy?"BUY":"SELL"), g_mobile_consecutive_fails + 1);
            
            // ★ Set building flag BEFORE calling OpenPositionsEx
            g_mobile_grid_building = true;
            g_mobile_grid_built = true;  // ★ Pre-emptively set to prevent re-entry
            g_mobile_check_cooldown = 100; // ★ INCREASED to 100 ticks (~10 seconds)
            ...
            // ★ INCREASED wait time for slower brokers
            Sleep(1000); // ← Increased from 500ms to 1000ms
            
            // ★ Verify that orders were actually placed
            Sleep(200); // ← Additional sync delay
            bool success = HasOurPositionsOrOrders();
            
            if(success)
            {
               g_mobile_consecutive_fails = 0; // Reset failure counter on success
               PrintFormat("✅ Grid auto-built successfully for mobile position ticket %I64u (Bottom Grid mode)", ticket);
            }
            else
            {
               g_mobile_consecutive_fails++;
               g_mobile_grid_built = false; // Mark as not built to allow retry
               g_mobile_check_cooldown = 30; // Short cooldown before retry
               PrintFormat("⚠️ Grid build failed (no orders detected) - will retry (attempt %d/3)", g_mobile_consecutive_fails);
            }
            
            // ★ Clear building flag
            g_mobile_grid_building = false;
         }
         else if(g_mobile_consecutive_fails >= 3)
         {
            PrintFormat("❌ Grid build failed after 3 attempts - manual intervention required");
            g_mobile_base_lot = 0.0; // Reset to prevent further auto-attempts
         }
      }
      else
      {
         ...
         if(g_mobile_base_lot > 0.0)
         {
            ...
            g_mobile_check_cooldown = 0;
            g_mobile_grid_building = false;
            g_mobile_consecutive_fails = 0;  // ★ Reset failure counter
         }
      }
   }
   else
...
//─────────────────────────────────────────────────────────────
// Periodic cleanup: Remove orphaned limit orders (safety net)
//─────────────────────────────────────────────────────────────
void PeriodicOrphanedOrderCleanup()
{
   // Check if we have EA positions
   bool has_ea_positions = false;
   ...
   // ★ If no EA positions AND (no mobile OR mobile closed), delete all EA orders
   if(!has_ea_positions && (!g_mobile_base_lot > 0.02 || !mobile_exists))
   {
      int order_count = 0;
      for(int i=OrdersTotal()-1; i>=0; --i)
      {
      ...
      // If orphaned orders exist, clean them up
      if(order_count > 0)
      {
         PrintFormat("★ Periodic cleanup: found %d orphaned limit orders, deleting...", order_count);
         ...
         PrintFormat("★ Periodic cleanup complete");
...
void OnTick()
{
   ...
   // ★ Periodic orphaned limit order cleanup (every 10 ticks)
   static int cleanup_counter = 0;
   cleanup_counter++;
   if(cleanup_counter >= 10)
   {
      cleanup_counter = 0;
      PeriodicOrphanedOrderCleanup();
   }
